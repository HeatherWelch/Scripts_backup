...
data=read.csv("F:/Climate_paper/GAM_1/FINAL_Summary_AUC_cuttoff.csv")
species<-levels(data$Species)  #this can commented out during the testing phase
# species<-file_path_sans_ext(ls(pattern="0.csv")) #this is commented out during the running phase
...
#   if (Summary_Stats_Spp$test_mean>=0.7){   ## should change this cuttoff based on the cuttoff chosen below
  spp_output_dir=paste(getwd(),"/",spp,sep="");dir.create(spp_output_dir)
  }


#creating a list of raster directories for looping and projecting
raster_dir="F:/Climate_paper/GAM_1/project_future"
setwd(raster_dir)
...
#   if (Summary_Stats$Model_AUC>=0.7){ ## set cutoff based on how much explained variance or AUC 
    ...
    dat<-read.csv(paste("F:/Climate_paper/GAM_1/species/",spp,".csv",sep=""))
...
for(spp in species){
  ...
  dat<-read.csv(paste("F:/Climate_paper/GAM_1/species/",spp,".csv",sep=""))
  final_object<-gam(p_a ~s(Depth,bs="ts")+s(Rugosity,bs="ts")+s(st,bs="ts")+s(bt,bs="ts")+s(SS,bs="ts")+s(bs,bs="ts")+s(sh,bs="ts"),family=binomial("logit"),data=dat,method="GCV.Cp")
  print(paste("GAM fit for ",spp,sep=""))
  for(d in dir_name){
    print(d)
    rasterStack=get(d)
    raster::predict(rasterStack,final_object,filename=paste("F:/Climate_paper/GAM_1/Species_Projections_all/",spp,"/",spp,"_",d,".tif",sep=""),fun=predict,format="GTiff", type="response",na.rm=TRUE,overwrite=TRUE,progress='text')
    print(paste("projection of ",spp," for ",d," finished",sep=""))
  }
}
