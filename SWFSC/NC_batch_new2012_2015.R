...
# wind
#netcdf=list.files(pattern="*ncdcOwDly*") #names of netcdffiles (2015)
netcdf=list.files(pattern="*ncdcOwDly*") #2012
netcdf=netcdf[grep("2012",netcdf)] #2012
...
for(nc in netcdf){
  ...
  for(n in names(tmp.df02)){
    print(n)
   if(file.exists(path1=paste(output_dir,"/",n,sep=""))){
    ...
    wind=raster::resample(rotate(r), template, method="bilinear")
...
# EKE
#netcdf=list.files(pattern="*msla-uv*")#names of netcdffiles #(2015)
netcdf=list.files("/Volumes/SeaGate/ERD_DOM/ncdf_ADD/AVISOraw/all-sat-DRT/uv/2012",pattern="*msla_uv*",full.names = TRUE) #(2012)
template_native=raster(netcdf[1])
...
for(nc in netcdf){
  ...
  for(n in names(tmp.df02)){
    ...
    if(file.exists(path)==FALSE){
      r=rotate(rasterize(tmp.df02,template_native,field=n,fun=mean)) # points to raster 
      name=paste("v_",n,sep="")
      print(name)
      assign(name,r)
    }
...
### l.eke
#years=as.character(seq(from=as.Date("2015-01-01"),to=as.Date("2015-07-31"),by="day")) #2015
years0=as.character(seq(from=as.Date("2012-01-01"),to=as.Date("2012-07-31"),by="day")) #2012  ## add december
years1=as.character(seq(from=as.Date("2012-12-01"),to=as.Date("2012-12-31"),by="day")) #2012  ## add december
years=unlist(list(years0,years1))
...
### SLA
netcdf=list.files("/Volumes/SeaGate/ERD_DOM/ncdf_ADD/AVISOraw/all-sat-DRT/h/2012",pattern="*msla_h*",full.names = TRUE) #(2012)
#netcdf=list.files(pattern="*msla_h*")#names of netcdffiles #2015
#netcdf=netcdf[1:212]
template_native=raster(netcdf[1])
...
for(nc in netcdf){
  ...
  for(n in names(tmp.df02)){
    ...
    if(file.exists(path)==FALSE){
      ...
      sla_ras=raster::resample(rotate(r), template, method="bilinear")# points to raster 
...
# CHLA
netcdf=list.files("/Volumes/SeaGate/ERD_DOM/ncdf",pattern="erdMBchla8day_LonPM180",full.names = T)  #2012
#netcdf=list.files(pattern="*erdMBchla8day*")#names of netcdffiles #2015
template_native=raster(netcdf[1])
...
for(nc in netcdf){
  ...
  tmp.mat <- matrix(tmp.vec.long, nrow = nlon * nlat, ncol = nt)
  print("Formatting column names")
  date <- as.character(as.POSIXlt(t,origin='1970-01-01',tz= "UTC"))
  print("Creating spatial dataframe")
  lonlat <- expand.grid(lon, lat)
  names(lonlat) <- c("lon","lat")
  tmp.df02 <- data.frame(tmp.mat)
  ...
  print("converting to raster")
  for(n in names(tmp.df02)){
    path=paste(output_dir,"/",n,"/l.blendChl.grd",sep="")
    if(file.exists(path)==FALSE){
      r=rasterize(tmp.df02,template,field=n,fun=mean)
      #r1=rotate(log(r+0.001)) ## 2015
      r1=log(r+0.001)
      writeRaster(r1,paste(output_dir,"/",n,"/l.blendChl",sep=""),overwrite=TRUE)
    }
  }
}
...
##sst
netcdf=list.files(pattern="*jplG1SST*")#names of netcdffiles
template_native=raster(netcdf[1])
...
for(nc in netcdf){
  ...
  ncin <- nc_open(ncc)
  print(ncin)
  dname="SST" # define variable of interest ########### change for each variable
  print("defining variables")
  lon <- ncvar_get(ncin, "longitude") # define longitude
  nlon <- dim(lon)
  lat <- ncvar_get(ncin, "latitude", verbose = F) # define latitude
